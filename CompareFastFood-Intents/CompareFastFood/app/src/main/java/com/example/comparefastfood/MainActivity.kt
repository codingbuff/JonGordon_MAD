...
import com.google.android.material.switchmaterial.SwitchMaterial
 ...
 class MainActivity : AppCompatActivity() {
     ...
     lateinit var resSpin : Spinner
     lateinit var mealRadio : RadioGroup
     lateinit var layoutRoot : ConstraintLayout
     lateinit var drinkRadio : RadioGroup
     lateinit var iceCreamCheck : CheckBox
     lateinit var cookieCheck : CheckBox
     lateinit var largeSwitch : Switch
     private var myCalorieresults = CalorieResults();
     lateinit var resourceButton : Button
     private var selectedRestaurantIdx = 0
     private var mealType = "Burger"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        //views
        resSpin = findViewById<Spinner>(R.id.spinner)
        mealRadio = findViewById<RadioGroup>(R.id.mealRadio)
        layoutRoot = findViewById<ConstraintLayout>(R.id.root_layout)
        drinkRadio = findViewById<RadioGroup>(R.id.drinkRadio)
        iceCreamCheck = findViewById<CheckBox>(R.id.iceCream)
        cookieCheck = findViewById<CheckBox>(R.id.cookie)
        largeSwitch = findViewById<Switch>(R.id.switch1)
        outputMsg = findViewById<TextView>(R.id.output)
        resourceButton = findViewById<Button>(R.id.resourceBtn)
        //mealId = mealRadio.checkedRadioButtonId
        //drinkId = drinkRadio.checkedRadioButtonId

        //event listener
        resourceButton.setOnClickListener {
            selectedRestaurantIdx = resSpin.selectedItemPosition
            val mealId = mealRadio.checkedRadioButtonId
            mealType = findViewById<RadioButton>(mealId).text.toString()
            //Log.i("meal type","$mealType")
            myCalorieresults.showResource(selectedRestaurantIdx,mealType)
            Log.i("resource URL:",myCalorieresults.url)

            //create intent
            val intent = Intent(this,CalorieActivity::class.java)
            intent.putExtra("calorieCount",myCalorieresults.calorieCount)
            intent.putExtra("nutritionURL",myCalorieresults.url)
...
//         val largeSwitch = findViewById<Switch>(R.id.switch1)
         ...
         burgerKingCal["Burger"] = 667 //whopper
         burgerKingCal["Crispy Chicken"] = 1245
         burgerKingCal["Nuggets"] = 224 //assuming 4 piece
...
//Testing retrieving indices from restaurant and meals
         val restIdx = resSpin.selectedItemPosition //restaurant index as an int
//         Log.i("restIdx", "$restIdx")
         ...
         if (mealId == -1) {
             ...
             myCalorieresults.setCalCount(calorieString)
     ...
     override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
         ...
         if((requestCode == REQUEST_CODE) && (resultCode == Activity.RESULT_OK)) {
             message += data?.getStringExtra("worthIt")
             updateUI()
         }
     }
